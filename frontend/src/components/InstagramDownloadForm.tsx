...
interface InstagramDownloadFormProps {
  onResult: (result: {
    success: boolean;
    title?: string;
    author?: string;
...
function isValidInstagramUrl(value: string): boolean {
  ...
  return /instagram\.com\/(?:p|reel|reels|tv)\/[A-Za-z0-9_-]+/i.test(trimmed) && trimmed.length > 20;
}

export function InstagramDownloadForm({ onResult, retryUrl, onRetryUrlConsumed }: InstagramDownloadFormProps) {
  ...
  const urlValid = url.trim() ? isValidInstagramUrl(url) : null;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!trimmed) {
      setError(t('instagram.errorInvalid'));
      return;
    }
    ...
    const downloadUrl = `${apiBase}/api/instagram`;
    try {
      ...
      if (typeof window !== 'undefined') {
        console.error('[CosmoVid] Instagram download API request failed:', message, { url: downloadUrl });
      }
      onResult({
        success: false,
        links: {},
        ...
        <div className="flex-1 min-w-0 relative">
          ...
          {url.trim().length > 0 && (
            <span
              ...
              aria-hidden
              title={urlValid ? t('instagram.urlValid') : t('instagram.urlInvalid')}
            >
              {urlValid === true ? (
                <span className="text-green-600 dark:text-green-400" aria-label={t('instagram.urlValid')}>✓</span>
              ) : urlValid === false ? (
                <span className="text-red-500" aria-label={t('instagram.urlInvalid')}>✗</span>
              ) : null}
            </span>
          )}
        </div>
